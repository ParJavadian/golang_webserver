...
func main() {
	...
	err := gen.RegisterAuthHandlerFromEndpoint(context.Background(), mux, "host.docker.internal:5052", []grpc.DialOption{grpc.WithInsecure()})
	if err != nil {
		log.Fatal(err)
	}
	...
	// setting up a dail up for gRPC service by specifying endpoint/target url
	err2 := gen.RegisterBizHandlerFromEndpoint(context.Background(), mux2, "host.docker.internal:5062", []grpc.DialOption{grpc.WithInsecure()})
	if err2 != nil {
		log.Fatal(err2)
	}
